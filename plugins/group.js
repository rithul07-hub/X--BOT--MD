...
const AUDIO_URL = "https://files.catbox.moe/jtiyuc.aac";
...
Sparky({
    ...
    try {
        ...
        if (!botJid) return;
        const botNumber = extractNumber(botJid.split(":")[0]);
        const mentioned = getMentions(m);
        console.log("MENTION DEBUG:", mentioned);
        let triggered = false;
        if (mentioned.length > 0) {
            triggered = mentioned.some(j =>
                extractNumber(j) === botNumber
            );
        }
        if (!triggered) {
            if (body.toLowerCase().includes("mee")) {
                triggered = true;
            }
        }
